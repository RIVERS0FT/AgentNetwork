...
> 当前实现映射：`agent_network/simulation_management/` 已统一运行配置、实例状态、最大持续时间、停止和资源分配；`agent_network/api/simulations.py` 仍是待迁移的固定轮次底层执行器。生命周期权威边界见 ADR-026，调度目标边界继续由 ADR-019 约束。
...
本设计已按 ADR-019 实施：配置请求显式创建 `SimulationRuntimeConfig`，每次运行拥有独立事件队列和稳定序号，Agent 按事件执行，容器资源与并发限制已落实，Dashboard 使用事件计数，实验清单升级为 v2。剧本加载不再读取或覆盖运行时调度配置。
