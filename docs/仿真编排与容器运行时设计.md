...
所有字段均由仿真创建或启动请求确定，并在启动前完成校验。它们不属于 `SceneDefinition`，也不应由剧本预览接口返回。

### 2.3 仿真状态枚举 `SimulationState`
...
3. 仿真编排从当前仿真实例读取运行参数；
4. 剧本查询和预览保持无副作用；
5. 测试覆盖读取剧本不会改变仿真运行状态；
6. 不保留从剧本元数据隐式覆盖运行配置的兼容分支。
