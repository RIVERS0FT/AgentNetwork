...
`SceneResource` 表示具有独立标识和生命周期的静态剧本目录资源。

| 函数 | 返回类型 | 职责 |
|---|---|---|
| `markTemporary(storageLocator, entryFiles)` | `void` | 记录临时目录定位信息和入口资源，并将状态设为 `temporary`。 |
| `applyValidation(result)` | `void` | 应用 `ValidationResult`，同步更新最近校验状态；校验失败时标记为 `invalid`。 |
| `markPersisted(storageLocator)` | `void` | 文件存储模块完成目录持久化后，将状态设为 `persisted`。 |
| `canDelete(hasActiveUsage)` | `boolean` | 当仿真管理模块返回不存在活动占用时返回 `true`。 |
| `markDeleting()` | `void` | 物理删除开始前将状态设为 `deleting`。 |
| `markDeleted()` | `void` | 文件存储模块完成目录删除后将状态设为 `deleted`。 |
...
`SceneArchive` 表示批量下载过程中产生的临时归档资源。归档用于传输，不改变平台内部以目录存储剧本的合同。

| 函数 | 返回类型 | 职责 |
|---|---|---|
| `startBuilding()` | `void` | 将归档状态设为 `building`。 |
| `includeScene(sceneKey)` | `void` | 将可读取剧本目录加入归档清单并更新包含数量。 |
...
classDiagram
    ...
    SceneArchive "1" o-- "1..*" SceneResource : 打包目录
```

## 6. 实现约束

- `SceneResource` 不得重新增加 `format` 字段；
- 不得重新定义 `SceneResourceFormat`；
- `storage_locator` 必须定位剧本目录；
- 下载归档只是传输输出，不是新的平台内部存储格式；
- `markPersisted` 不能代替目录持久化；
- `markDeleted` 不能代替目录物理删除；
