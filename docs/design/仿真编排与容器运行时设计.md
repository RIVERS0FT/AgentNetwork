...
> 状态：已实施。本文定义持续时间约束、事件到达即调度的仿真运行模型及其当前实现边界。

> 当前实现映射：`agent_network/simulation_management/` 已统一运行配置、实例状态、最大持续时间、停止、资源分配和每次运行的显式执行上下文；`agent_network/api/simulations.py` 是无 pending 状态的过渡执行器，只接受目标运行的显式参数。生命周期权威边界见 ADR-026、ADR-031，调度边界继续由 ADR-019 约束。
...
| `scene_definition` | `SceneDefinition` | 是 | 配置阶段加载并校验的静态剧本快照，仅供内部执行。 |
| `execution_config` | object | 是 | 本次运行的非公开后端执行配置，不通过 API 序列化。 |
| `seed` | integer | 是 | 本次运行显式持有并传递的随机种子。 |
...
| `agent_startup_timeout_seconds` | integer | 是 | 全部已分配 Agent Server 通过 `/status` 就绪检查的共享最长等待时间，默认 60 秒。 |
...
classDiagram
    ...
    class SimulationRuntimeConfig {
        ...
        +integer agent_startup_timeout_seconds
...
当前代码使用单一 `SimulationManager` 运行注册表限制并发。`simulation_management.state` 中的进程级字段只服务 WebSocket、日志接收和调试展示，是可观测投影，不能决定剧本占用、生命周期或执行输入，也不构成永久外部合同。
...
- 校验 Agent 的 `native_capabilities`、原生子 Agent 配额和不可授权的原生消息能力；
- 校验通信拓扑及网络参数；
...
runtime_config.agent_startup_timeout_seconds
...
5. 将已校验的 `SceneDefinition`、seed 和非公开执行配置绑定到该 `SimulationRun`；
6. 预留系统资源；
7. 重置并分配容器，并应用 CPU、内存、进程数等限制；
8. 在共享的 `agent_startup_timeout_seconds` 窗口内轮询所有 Agent 的 `GET /status`，容器提前退出或窗口超时均视为分配失败；
9. 对已就绪 Agent 执行 reset，构造 Agent 目录和本次运行局部的通信矩阵并注入控制面配置；
10. 创建本次仿真的事件队列并写入初始任务事件；
11. 启动日志 session；
12. 创建 experiment manifest，记录 `event_driven` 调度模式、持续时间配置、启动超时、资源限制、原生策略哈希和后端精确版本；
13. 配置网络 profile；
14. 通过显式注入的抓包入口启动所有 Agent 的 PCAP；
15. 检查启动完整性；
16. 将仿真状态置为 `running` 并启动事件调度器。

资源无法预留、资源限制无法应用、任一 Agent Server 未在共享窗口内就绪、已请求的网络仿真无法配置，或任一 Agent 抓包无法启动时，仿真不得继续运行。
...
5. 全部已分配容器共享一个启动窗口，轮询 `GET /status`，不得使用固定 sleep 或仅检查 Docker running；
6. 容器进入 `exited`/`dead` 时立即记录状态与退出码，窗口超时时记录 Agent URL 和最后一次连接错误；
7. 只有就绪 Agent 才能执行 `/reset` 和 `/communication/configure`，任一失败时回滚本次全部容器；
8. 根据 `max_parallel_agents` 限制同时执行的 Agent 任务数量；
9. 无法分配、无法应用限制或无法就绪时记录 assignment error，并以 `assignment_failed` 结束仿真。

容器名不是逻辑 Agent ID。业务日志必须使用运行上下文中的逻辑 `agent_id`。

原生子 Agent 不分配新容器。它继承父 Agent 的容器 ID、容器 URL 和网络身份，但必须使用独立的 backend session/run ID，并在 `AgentRegistry` 中标记 `runtime_kind=<backend>_native` 和 `parent_agent_id`。
...
Claude Code 和 OpenClaw 可以在一次 Agent 事件处理中拉起后端原生子 Agent，但子 Agent 只承担内部委派：

1. 执行前按父 Agent 的 `native_capabilities` 检查 `agent.spawn`；
2. 同时检查最大子 Agent 数、最大并行数、最大深度和 `child_can_spawn`；
3. 父子共享容器，子 Agent 使用独立 session；
4. lifecycle Hook 把 running 与终态写入 `application.v12`，并投影到 AgentRegistry；
5. 子 Agent 不得使用原生消息 Tool 与其他平台 Agent 通信；需要通信时由父 Agent 通过 MCP `send_message` 进入 CommManager；
6. Tool 请求或子 Agent 缺少终态时，本次 experiment quality audit 失败。
...
3. 将仍在运行的原生子 Agent 标记为取消、终止、超时、reset 或 killed，并完成 lifecycle 审计；
4. 停止所有 Agent 抓包；
5. 清理所有 Agent 的 `tc` 配置；
6. 将容器状态恢复为 `idle`，并清除本次仿真的资源限制、原生子 Agent 投影和运行上下文；
7. 完成 experiment manifest，记录持续时间、已处理事件数、停止原因、资源结果、原生策略与子 Agent 树；
8. 执行 `audit_session`；
9. 完成或失败当前 `SimulationRun`；
10. 释放并发容量和预留的系统资源。
...
本设计已按 ADR-019、ADR-026、ADR-031、ADR-032 和 ADR-035 实施：配置请求显式创建 `SimulationRuntimeConfig`，每次运行拥有独立场景快照、seed、非公开执行配置、事件队列和稳定序号；Agent 按事件执行，容器资源与并发限制已落实，容器控制面配置前执行应用层就绪屏障，Dashboard 使用事件计数，实验清单升级为 v3。剧本加载不再读取或覆盖运行时调度配置；Claude/OpenClaw 原生能力和子 Agent 已纳入统一授权、审计和质量门槛。

## 12. 2026-07-23 复用容器资源更新兼容性修正

`ap_deployment` 在复用已运行容器时曾出现全部 Agent 分配失败。实验清单记录的直接原因是 Docker SDK 7.1.0 的 `Container.update()` 将 `nano_cpus` 传给不支持该关键字的 `ContainerApiMixin.update_container()`。同一 SDK 的高层更新方法也不接受 `pids_limit`，因此仅替换 CPU 参数会在进程数限制处再次失败。

容器创建与复用更新现使用两套明确的资源转换：

- 新建容器继续向 `containers.run()` 传入 `nano_cpus`、`mem_limit` 和 `pids_limit`；
- 复用容器将同样的限制转换为 Docker Engine `POST /containers/{id}/update` 负载中的 `NanoCpus`、`Memory` 和 `PidsLimit`；
- Engine 返回非成功状态时继续包装为带容器名的 `could not apply resources` 错误，任一限制无法应用时仍禁止仿真启动。

回归覆盖同时校验创建参数、Engine 更新负载和复用容器的实际调用路径。实机兼容性检查已对 `ag-o10` 重新应用其现有的 1 CPU、1024 MiB 内存和 128 进程限制，Engine 返回 HTTP 200。
