...
8. 构造 Agent 目录和本次运行局部的通信矩阵；
9. 创建本次仿真的事件队列并写入初始任务事件；
10. 启动日志 session；
11. 创建 experiment manifest，记录 `event_driven` 调度模式、持续时间配置、资源限制、原生策略哈希和后端精确版本；
12. 配置网络 profile；
13. 通过显式注入的抓包入口启动所有 Agent 的 PCAP；
14. 检查启动完整性；
15. 将仿真状态置为 `running` 并启动事件调度器。
...
本设计已按 ADR-019、ADR-026、ADR-031 和 ADR-032 实施：配置请求显式创建 `SimulationRuntimeConfig`，每次运行拥有独立场景快照、seed、非公开执行配置、事件队列和稳定序号；Agent 按事件执行，容器资源与并发限制已落实，Dashboard 使用事件计数，实验清单升级为 v3。剧本加载不再读取或覆盖运行时调度配置；Claude/OpenClaw 原生能力和子 Agent 已纳入统一授权、审计和质量门槛。
