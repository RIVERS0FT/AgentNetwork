...
| AR-SCENE-11 | 剧本：运行数据隔离 | SR-SCENE-01～05 | 剧本定义、查询、预览和管理过程不得包含或维护仿真运行数据模型。 |
...
sequenceDiagram
    ...
    Store-->>SceneMgr: 剧本资源
    SceneMgr->>Parser: 只读解析并生成预览
    Parser-->>SceneMgr: ScenePreview
    SceneMgr-->>Web: 返回静态预览
    Web-->>User: 展示剧本信息
```
...
4. 实现批量上传及失败清理；
5. 实现批量下载和归档；
6. 实现批量删除，并通过 `IF-SIM-01` 获取占用布尔值；
7. 补充日志、测试和文档映射。