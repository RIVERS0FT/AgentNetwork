# 剧本管理设计

> 状态：设计阶段。本文记录剧本上传、查询、下载、删除和详情查看的需求边界、逻辑模块、接口、架构要求和交互流程。

## 1. 设计范围

剧本管理负责静态剧本资源进入平台后的完整管理流程。

| SR-ID | SR名称 | SR描述 |
|---|---|---|
| SR-SCENE-01 | 剧本管理：上传剧本 | 平台支持批量上传剧本，对每个剧本进行临时保存、合法性校验、正式持久化和失败清理。 |
| SR-SCENE-02 | 剧本管理：下载剧本 | 平台支持批量下载剧本，将可读取剧本归档，并分别返回缺失或读取失败项。 |
| SR-SCENE-03 | 剧本管理：删除剧本 | 平台支持批量删除剧本，并在物理删除前检查运行占用状态。 |
| SR-SCENE-04 | 剧本管理：查看剧本详情 | 平台支持在不启动仿真的情况下读取单个剧本的完整静态详情。 |
| SR-SCENE-05 | 剧本管理：查询剧本列表 | 平台支持查询已保存剧本，并按照加入顺序返回只显示剧本名称的列表。 |

列表与详情边界：

- 剧本列表只显示剧本名称；
- 列表接口返回 `SceneListItem[]`，其中 `scene_key` 仅用于定位，前端只渲染 `title`；
- 单个剧本详情统一返回 `SceneSummary`；
- 不再定义或返回 `ScenePreview`；
- 查询不提供可配置排序，固定按照剧本加入平台的先后顺序输出；
- 列表和详情均不得读取、返回或修改仿真运行数据。

## 2. 领域边界

`SceneDefinition` 只描述静态剧本内容：

- 剧本基本信息；
- Agent 定义；
- Skill 定义；
- Tool 定义；
- Agent 对 Skill 和 Tool 的引用；
- 通信拓扑及网络参数。

以下内容属于仿真编排领域，不属于剧本管理：

- 仿真实例；
- 最大轮数和停滞终止阈值；
- 运行模式和随机种子；
- 当前轮次和仿真状态；
- 运行引用和剧本占用详情；
- 容器分配、日志会话、PCAP、manifest 和终止原因。

剧本删除需要调用仿真管理模块确认是否存在活动占用，但剧本管理不保存或复制仿真运行数据模型。

## 3. 当前代码基础与设计缺口

当前代码已具备：

- 从持久化目录发现可用剧本；
- 按剧本标识读取定义文件；
- 将剧本基本信息、Agent 引用和通信拓扑解析为内存对象；
- 在 Agent 运行期间按需读取 Skill 和场景 Tool 资源。

当前尚缺少完整的：

- 批量上传与逐项结果汇总；
- 临时存储、合法性校验、正式持久化和失败清理；
- 独立 Skill 定义集合和 Tool 定义集合；
- 批量下载与归档；
- 批量删除与占用保护；
- 只返回剧本名称的列表合同；
- 统一使用 `SceneSummary` 的单个剧本详情合同；
- 无运行副作用的列表和详情读取流程。

当前剧本加载器仍会读取轮数配置并修改全局仿真状态，这是待迁移实现，不属于目标剧本管理设计。

## 4. 逻辑模块

| 模块 | 主要职责 | 关联SR |
|---|---|---|
| Web前端模块 | 提交上传、查询、下载、删除和详情请求；展示名称列表、详情和处理结果 | SR-SCENE-01～05 |
| 剧本管理模块 | 编排剧本管理流程；维护批量任务上下文；汇总成功项和失败项 | SR-SCENE-01～05 |
| 文件存储模块 | 临时保存、持久化、定位、读取、删除和清理剧本资源 | SR-SCENE-01～05 |
| 剧本解析校验模块 | 解析静态剧本；分别构造 Skill 和 Tool 定义；生成 `SceneSummary` 并校验引用和拓扑 | SR-SCENE-01、04、05 |
| 文件归档模块 | 将多个可下载剧本组合为归档资源 | SR-SCENE-02 |
| 仿真管理模块 | 对外提供“目标剧本是否存在活动占用”的查询能力 | SR-SCENE-03 |

设计约束：

- 剧本管理模块只负责编排，不直接实现文件解析、归档或仿真运行；
- 文件存储模块不理解剧本业务内容；
- 剧本解析与合法性校验属于同一逻辑模块；
- Skill 与 Tool 分别解析、分别存储、分别校验；
- 当前不引入独立元数据数据库；
- 固定加入顺序由 `SceneResource.created_at` 表达，不向用户提供排序参数；
- 列表只构造 `SceneListItem`，不得为列表逐个解析完整详情；
- 单个详情统一构造 `SceneSummary`，不得再构造 `ScenePreview`；
- 剧本模块不得维护仿真运行配置、仿真实例、运行引用集合或运行状态；
- 删除保护只消费仿真管理模块的查询结果，不把仿真内部数据复制到剧本领域。

## 5. 模块接口

### 5.1 Web前端调用的业务接口

| 接口ID | 接口名称 | 提供模块 | 调用模块 | 关联SR | 主要输入 | 主要输出 |
|---|---|---|---|---|---|---|
| IF-SCENE-01 | 批量上传剧本 | 剧本管理模块 | Web前端模块 | SR-SCENE-01 | 多个剧本资源 | 各剧本上传结果和失败原因 |
| IF-SCENE-02 | 查询剧本列表 | 剧本管理模块 | Web前端模块 | SR-SCENE-05 | 过滤和分页条件 | 按加入顺序排列的 `SceneListItem[]` |
| IF-SCENE-03 | 批量下载剧本 | 剧本管理模块 | Web前端模块 | SR-SCENE-02 | 多个剧本标识 | 归档资源和逐项结果 |
| IF-SCENE-04 | 批量删除剧本 | 剧本管理模块 | Web前端模块 | SR-SCENE-03 | 多个剧本标识 | 各剧本删除结果 |
| IF-SCENE-05 | 查看剧本详情 | 剧本管理模块 | Web前端模块 | SR-SCENE-04 | 剧本标识 | `SceneSummary` 或失败原因 |

### 5.2 内部接口

| 接口ID | 接口名称 | 提供模块 | 调用模块 | 关联SR | 主要职责 |
|---|---|---|---|---|---|
| IF-STORE-01 | 临时保存剧本 | 文件存储模块 | 剧本管理模块 | SR-SCENE-01 | 保存待校验资源 |
| IF-STORE-02 | 持久化剧本 | 文件存储模块 | 剧本管理模块 | SR-SCENE-01 | 将校验通过资源转入正式存储 |
| IF-STORE-03 | 定位剧本资源 | 文件存储模块 | 剧本管理模块 | SR-SCENE-02～05 | 根据剧本标识定位资源 |
| IF-STORE-04 | 读取剧本资源 | 文件存储模块 | 剧本管理模块、剧本解析校验模块 | SR-SCENE-02、04 | 读取剧本定义和关联资源 |
| IF-STORE-05 | 删除剧本资源 | 文件存储模块 | 剧本管理模块 | SR-SCENE-03 | 删除指定剧本持久化资源 |
| IF-STORE-06 | 清理临时数据 | 文件存储模块 | 剧本管理模块 | SR-SCENE-01 | 清理失败产生的临时数据 |
| IF-STORE-07 | 查询剧本名称 | 文件存储模块 | 剧本管理模块 | SR-SCENE-05 | 扫描持久化资源，按加入顺序返回标识和名称 |
| IF-PARSER-01 | 校验剧本 | 剧本解析校验模块 | 剧本管理模块 | SR-SCENE-01 | 校验静态结构、Agent、Skill、Tool 和拓扑 |
| IF-PARSER-02 | 解析剧本详情 | 剧本解析校验模块 | 剧本管理模块 | SR-SCENE-04 | 生成 `SceneDefinition`、校验结果和 `SceneSummary` |
| IF-ARCHIVE-01 | 创建剧本归档 | 文件归档模块 | 剧本管理模块 | SR-SCENE-02 | 将多个剧本组合为归档资源 |
| IF-SIM-01 | 查询剧本占用状态 | 仿真管理模块 | 剧本管理模块 | SR-SCENE-03 | 输入 `scene_key`，返回是否存在活动占用；运行详情由仿真域内部维护 |

所有接口必须明确输入所有权、资源生命周期、失败语义、批量项结果和可观测事件。

## 6. 架构要求

| AR-ID | AR名称 | 关联SR | AR描述 |
|---|---|---|---|
| AR-COM-01 | 通用：批量文件操作 | SR-SCENE-01、02、03 | 系统应一次提交并分别处理多个剧本资源。 |
| AR-COM-03 | 通用：文件持久化 | SR-SCENE-01、02、04、05 | 系统应提供临时存储、正式持久化、定位和读取能力。 |
| AR-COM-04 | 通用：批量结果反馈 | SR-SCENE-01、02、03 | 系统应分别返回每个剧本的成功或失败结果。 |
| AR-SCENE-01 | 剧本：合法性校验 | SR-SCENE-01 | 系统应校验静态剧本结构、Agent、Skill、Tool、引用关系和通信拓扑。 |
| AR-SCENE-02 | 剧本：失败数据清理 | SR-SCENE-01 | 校验失败或处理异常时应清理对应临时数据。 |
| AR-SCENE-03 | 剧本：归档下载 | SR-SCENE-02 | 系统应将可用剧本打包为统一归档资源。 |
| AR-SCENE-04 | 剧本：资源删除 | SR-SCENE-03 | 系统应定位并删除指定剧本资源。 |
| AR-SCENE-05 | 剧本：删除保护 | SR-SCENE-03 | 仿真管理模块确认存在活动占用时不得物理删除剧本。 |
| AR-SCENE-06 | 剧本：结构化解析 | SR-SCENE-04 | 系统应解析基本信息、Agent、Skill、Tool、任务和通信关系并生成 `SceneSummary`。 |
| AR-SCENE-07 | 剧本：只读详情 | SR-SCENE-04 | 查看详情不得启动仿真、分配容器或修改仿真状态。 |
| AR-SCENE-08 | 剧本：详情异常反馈 | SR-SCENE-04 | 不存在、不可读取或解析失败时应返回明确原因。 |
| AR-SCENE-09 | 剧本：名称列表 | SR-SCENE-05 | 系统应只返回用于列表展示和目标选择的剧本名称项。 |
| AR-SCENE-10 | 剧本：Skill与Tool分离 | SR-SCENE-01、04 | `SceneDefinition` 应分别保存 Skill 和 Tool 定义集合。 |
| AR-SCENE-11 | 剧本：运行数据隔离 | SR-SCENE-01～05 | 剧本定义、列表、详情和管理过程不得包含或维护仿真运行数据模型。 |
| AR-SCENE-12 | 剧本：列表详情分离 | SR-SCENE-04、05 | 列表使用 `SceneListItem`，详情使用 `SceneSummary`，不得定义 `ScenePreview`。 |
| AR-SCENE-13 | 剧本：固定加入顺序 | SR-SCENE-05 | 列表固定按剧本加入平台的先后顺序输出，不提供用户可配置排序。 |

## 7. 逻辑数据模型

```mermaid
classDiagram
    class 剧本资源 {
        +剧本标识
        +剧本名称
        +资源状态
        +校验状态
        +加入时间
        +存储定位信息
    }

    class 剧本定义 {
        +基本信息
        +Agent定义集合
        +Skill定义集合
        +Tool定义集合
        +通信拓扑
    }

    class Agent定义 {
        +Agent标识
        +角色
        +目标
        +后端类型
        +Skill引用集合
        +Tool授权集合
    }

    class Skill定义 {
        +Skill标识
        +入口资源
        +存储定位信息
        +可用状态
    }

    class Tool定义 {
        +Tool标识
        +模块定位信息
        +可执行状态
    }

    class 通信关系 {
        +源Agent
        +目标Agent
        +通道标识
        +网络参数
    }

    class 校验结果 {
        +是否通过
        +错误列表
        +警告列表
    }

    class 剧本详情 {
        +基本信息
        +资源与校验状态
        +Agent列表
        +Skill列表
        +Tool列表
        +通信关系
        +校验结果
    }

    class 剧本查询条件 {
        +关键字
        +资源状态
        +校验状态
        +分页参数
    }

    class 剧本列表项 {
        +剧本标识
        +剧本名称
    }

    class 批量处理任务 {
        +任务类型
        +请求数量
        +成功数量
        +失败数量
    }

    class 批量项结果 {
        +剧本标识
        +处理状态
        +失败原因
    }

    class 归档资源 {
        +归档状态
        +包含剧本数量
        +资源定位信息
    }

    剧本资源 "1" *-- "1" 剧本定义 : 包含
    剧本定义 "1" *-- "1..*" Agent定义 : 定义
    剧本定义 "1" *-- "0..*" Skill定义 : 分开存储
    剧本定义 "1" *-- "0..*" Tool定义 : 分开存储
    剧本定义 "1" *-- "0..*" 通信关系 : 定义
    Agent定义 --> Skill定义 : 引用
    Agent定义 --> Tool定义 : 授权
    剧本定义 --> 校验结果 : 校验产生
    剧本定义 --> 剧本详情 : 转换为SceneSummary
    剧本详情 *-- Agent定义 : 展示
    剧本详情 *-- Skill定义 : 展示
    剧本详情 *-- Tool定义 : 展示
    剧本详情 *-- 通信关系 : 展示
    剧本查询条件 --> 剧本列表项 : 查询返回
    批量处理任务 "1" *-- "1..*" 批量项结果 : 汇总
    归档资源 "1" o-- "1..*" 剧本资源 : 打包
```

该模型只包含剧本管理数据。仿真占用、运行引用、仿真实例和运行配置统一定义在《仿真编排与容器运行时设计》中。

## 8. 时序设计

### 8.1 批量上传

```mermaid
sequenceDiagram
    actor User as 用户
    participant Web as Web前端模块
    participant SceneMgr as 剧本管理模块
    participant Store as 文件存储模块
    participant Parser as 剧本解析校验模块

    User->>Web: 上传多个剧本
    Web->>SceneMgr: 提交批量上传请求
    loop 逐个处理
        SceneMgr->>Store: 临时保存
        Store-->>SceneMgr: 临时资源
        SceneMgr->>Parser: 解析并校验静态剧本
        alt 校验通过
            Parser-->>SceneMgr: SceneDefinition和校验结果
            SceneMgr->>Store: 正式持久化并记录加入时间
            Store-->>SceneMgr: 保存成功
        else 校验失败
            Parser-->>SceneMgr: 错误列表
            SceneMgr->>Store: 清理临时数据
        end
    end
    SceneMgr-->>Web: 返回逐项结果
    Web-->>User: 展示成功项和失败项
```

### 8.2 批量下载

```mermaid
sequenceDiagram
    actor User as 用户
    participant Web as Web前端模块
    participant SceneMgr as 剧本管理模块
    participant Store as 文件存储模块
    participant Archive as 文件归档模块

    User->>Web: 选择多个剧本下载
    Web->>SceneMgr: 提交批量下载请求
    loop 逐个处理
        SceneMgr->>Store: 定位并读取剧本
        alt 可读取
            Store-->>SceneMgr: 剧本资源
            SceneMgr->>Archive: 加入归档
        else 不可读取
            Store-->>SceneMgr: 失败原因
            SceneMgr->>SceneMgr: 记录失败项
        end
    end
    SceneMgr->>Archive: 构建归档
    Archive-->>SceneMgr: 归档结果
    SceneMgr-->>Web: 返回归档和逐项结果
    Web-->>User: 下载并展示失败项
```

### 8.3 批量删除

```mermaid
sequenceDiagram
    actor User as 用户
    participant Web as Web前端模块
    participant SceneMgr as 剧本管理模块
    participant SimMgr as 仿真管理模块
    participant Store as 文件存储模块

    User->>Web: 选择多个剧本删除
    Web->>SceneMgr: 提交批量删除请求
    loop 逐个处理
        SceneMgr->>SimMgr: 查询目标剧本是否存在活动占用
        alt 存在活动占用
            SimMgr-->>SceneMgr: has_active_usage=true
            SceneMgr->>SceneMgr: 记录SCENE_IN_USE
        else 无活动占用
            SimMgr-->>SceneMgr: has_active_usage=false
            SceneMgr->>Store: 删除剧本资源
            Store-->>SceneMgr: 删除结果
        end
    end
    SceneMgr-->>Web: 返回逐项结果
    Web-->>User: 展示删除结果
```

剧本管理只消费 `has_active_usage`，不接收或保存仿真实例、运行引用集合和运行状态详情。

### 8.4 查看剧本详情

```mermaid
sequenceDiagram
    actor User as 用户
    participant Web as Web前端模块
    participant SceneMgr as 剧本管理模块
    participant Store as 文件存储模块
    participant Parser as 剧本解析校验模块

    User->>Web: 选择剧本并查看详情
    Web->>SceneMgr: 提交scene_key
    SceneMgr->>Store: 读取静态剧本资源
    Store-->>SceneMgr: 剧本资源
    SceneMgr->>Parser: 只读解析完整静态详情
    Parser-->>SceneMgr: SceneSummary
    SceneMgr-->>Web: 返回SceneSummary
    Web-->>User: 展示剧本详情
```

### 8.5 查询剧本列表

```mermaid
sequenceDiagram
    actor User as 用户
    participant Web as Web前端模块
    participant SceneMgr as 剧本管理模块
    participant Store as 文件存储模块

    User->>Web: 打开剧本列表
    Web->>SceneMgr: 提交过滤和分页条件
    SceneMgr->>Store: 按加入顺序查询剧本标识和名称
    Store-->>SceneMgr: SceneListItem列表
    SceneMgr-->>Web: 返回SceneListItem列表
    Web-->>User: 仅展示剧本名称
```

列表查询不得为了显示名称而逐个解析 Agent、Skill、Tool 或拓扑详情。

## 9. 失败语义

- 批量操作逐项处理，不因单项失败回滚已成功项；
- 上传校验失败时清理对应临时资源；
- 下载允许部分剧本未进入归档，并返回省略原因；
- 删除遇到活动占用时返回 `SCENE_IN_USE`，不得物理删除；
- 详情不存在、不可读取或解析失败时返回明确错误；
- 列表查询失败不得返回不完整的详情结构；
- 列表和详情失败不得修改剧本资源或仿真状态。

## 10. 实现顺序

1. 将剧本加载器改为无运行副作用的静态解析器；
2. 构造独立 `SkillDefinition[]` 和 `ToolDefinition[]`；
3. 实现按加入顺序返回 `SceneListItem[]` 的名称列表；
4. 实现统一返回 `SceneSummary` 的详情接口；
5. 删除 `ScenePreview` 及相关生产、消费和测试代码；
6. 实现批量上传及失败清理；
7. 实现批量下载和归档；
8. 实现批量删除，并通过 `IF-SIM-01` 获取占用布尔值；
9. 补充日志、测试和文档映射。
