...
| [ADR-030-设计驱动与可追溯变更流程.md](ADR-030-设计驱动与可追溯变更流程.md) | 要求全部修改同步留下设计记录，并建立设计、实现、验证和提交的证据链 |
...
4. 全部受版本控制的修改，以及影响复现的本地操作配置修改，都必须按 ADR-030 更新本目录中的相关设计记录。
...
8. 没有设计记录、实现映射或实际验证证据的修改视为未完成，不得合并。
