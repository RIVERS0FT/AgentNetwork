...
| [ADR-029-设计驱动与可追溯变更流程.md](ADR-029-设计驱动与可追溯变更流程.md) | 要求全部修改同步留下设计记录，并建立设计、实现、验证和提交的证据链 |
