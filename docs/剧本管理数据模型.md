...
- 剧本列表接口不接收搜索、过滤、分页或排序参数，直接返回全部已保存剧本；
- 剧本列表固定按照剧本加入平台的先后顺序输出；
...
剧本列表没有请求参数模型。打开列表时直接返回全部已保存剧本，不提供搜索、过滤、分页或排序参数。

### 7.1 剧本列表项结构体 `SceneListItem`
...
- 返回类型为 `SceneListItem[]`；
- 返回全部已保存剧本；
- 固定按照 `SceneResource.created_at` 从早到晚输出；
- 不存在 `SceneQuery`；
- 列表调用不得携带 `keyword`、状态过滤、`offset`、`limit` 或排序字段；
- 列表不得为了显示名称而逐个解析完整剧本详情。
...
classDiagram
    ...
    SceneResource --> SceneListItem : 提取名称列表项
```
...
- 不得恢复 `SceneQuery`，也不得为列表接口增加搜索、过滤、分页或排序参数；
- `SceneDefinition` 已独立保存 Agent、Skill、Tool、Task 和拓扑定义集合；
- 列表接口无查询参数并返回全部 `SceneListItem[]`；
- 详情接口统一返回 `SceneSummary`，不返回原始文件或内部资源标识；
- 剧本加载为只读静态解析，不修改仿真运行状态。
