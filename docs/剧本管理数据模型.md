...
- `SceneDefinition` 和 `ScenePreview` 不包含仿真轮数、终止阈值、随机种子、运行模式、当前轮次或仿真状态；
...
classDiagram
    ...
    SceneDefinition --> ScenePreview : 生成预览
    ScenePreview "1" *-- "1" SceneSummary : 包含摘要
    ScenePreview "1" *-- "1..*" AgentDefinition : 展示Agent
    ScenePreview "1" *-- "0..*" SkillDefinition : 展示Skill
    ScenePreview "1" *-- "0..*" ToolDefinition : 展示Tool
    ScenePreview "1" *-- "0..*" CommunicationRelation : 展示拓扑
    ScenePreview "1" o-- "0..1" ValidationResult : 附带校验
...
当前剧本加载流程还会修改全局仿真状态。该行为不属于剧本管理模型，应迁移到仿真创建和启动流程。