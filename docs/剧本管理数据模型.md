...
- 平台同一时间只允许一个仿真运行；剧本占用状态采用单实例语义，不维护活跃仿真数量或会话列表。
...
### 5.1 剧本占用状态结构体 `SceneUsageStatus`

平台同一时间只允许一个仿真运行。删除检查读取全局仿真状态，并判断当前运行剧本是否为待删除剧本。

| 字段 | 类型 | 必填 | 描述 |
|---|---|---:|---|
| `scene_key` | string | 是 | 被检查的剧本标识。 |
| `occupied` | boolean | 是 | 当前唯一运行仿真是否依赖该剧本。 |
| `simulation_running` | boolean | 是 | 平台当前是否存在运行中的仿真。 |
| `running_scene_key` | string | 否 | 当前运行仿真使用的剧本标识；无运行仿真时为空。 |
...
```text
occupied = simulation_running AND running_scene_key == scene_key
```

因此：

- 当前没有仿真运行时，任一剧本均不因运行占用而被阻止删除；
- 其他剧本正在运行时，只阻止删除该运行剧本，不阻止删除未被使用的剧本；
- 不维护 `active_count` 或 `active_session_ids`，因为平台不存在并行仿真实例。
...
当前全局状态使用单一的 `simulation_active` 和 `current_scene_name` 表达平台运行状态，因此 `SceneUsageStatus` 必须采用单实例占用语义。
