...
- 优雅停止先禁止后续轮次，逐 Agent 取消尚未执行的 A2A Task，使已登记的 Push 回调能够收到取消终态，然后等待运行清理完成。
...
统一生命周期、持续时间控制和资源限制已经实施；底层执行器仍使用旧固定轮次循环。该调度差距继续受 ADR-019 约束，后续应迁移为事件驱动调度，不得把当前轮次循环写成新的权威合同。
